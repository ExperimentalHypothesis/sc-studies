...
SCString log_sc_ym(SCStudyInterfaceRef sc) {
    ...
    SCString log;
    log.Format("Cena: %f, Date and Time: %i-%i-%i  %0.2i:%0.2i:%0.2i | Daily range (in ticks): %f | ATR: %0.04f", ym.entry_price, ym.year, ym.month, ym.day, ym.hour, ym.minute, ym.second, ym.range, ym.atr_val);
    return log;
}
...
SCSFExport scsf_main_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc) {
    ...
    if (return_val_sum <= MIN_SUM_FOR_SHORT // -6
        && return_val_from_es < 0 && return_val_from_nq < 0 && return_val_from_rty < 0 && return_val_from_ym > 0) {
        bool_ready_to_trade_LONG = 1;
        pv_iml_valid_from_dt = sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar];
    }
    // IML context for --------SHORT
    if (return_val_sum >= MIN_SUM_FOR_LONG // 6
        && return_val_from_es > 0 && return_val_from_nq > 0 && return_val_from_rty > 0 && return_val_from_ym < 0) {
