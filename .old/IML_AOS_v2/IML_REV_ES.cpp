...
SCString log_sc_es(SCStudyInterfaceRef sc) {
    ...
    double entry_dt = current_postion.LastFillDateTime.GetAsDouble();

    DATETIME_TO_YMDHMS(entry_dt, es.year, es.month, es.day, es.hour, es.minute, es.second);
    SCString log;
    log.Format("Cena: %f, Date and Time: %i-%i-%i  %0.2i:%0.2i:%0.2i | Daily range (in ticks): %f | ATR: %0.04f", es.entry_price, es.year, es.month, es.day, es.hour, es.minute, es.second, es.range, es.atr_val);
    return log;
}
...
SCSFExport scsf_MAIN_ES_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc) {
    ...
    float return_val_from_es = sc.GetPersistentFloatFromChartStudy(sc.Input[0].GetChartNumber(), sc.Input[0].GetStudyID(), 0);
    float return_val_from_ym = sc.GetPersistentFloatFromChartStudy(sc.Input[1].GetChartNumber(), sc.Input[1].GetStudyID(), 0);
    float return_val_from_rty = sc.GetPersistentFloatFromChartStudy(sc.Input[2].GetChartNumber(), sc.Input[2].GetStudyID(), 0);
    float return_val_from_nq = sc.GetPersistentFloatFromChartStudy(sc.Input[3].GetChartNumber(), sc.Input[3].GetStudyID(), 0);
    double daily_high = sc.GetPersistentDoubleFromChartStudy(sc.ChartNumber, sc.Input[5].GetStudyID(), 0);
    double daily_low = sc.GetPersistentDoubleFromChartStudy(sc.ChartNumber, sc.Input[5].GetStudyID(), 1);
    double daily_mid = sc.GetPersistentDoubleFromChartStudy(sc.ChartNumber, sc.Input[5].GetStudyID(), 2);
    double daily_low_mid = sc.GetPersistentDoubleFromChartStudy(sc.ChartNumber, sc.Input[5].GetStudyID(), 3);
    double daily_high_mid = sc.GetPersistentDoubleFromChartStudy(sc.ChartNumber, sc.Input[5].GetStudyID(), 4);
    ...
    auto iml_valid_till_dt = static_cast<SCDateTime>(pv_iml_valid_from_dt + SCDateTime::MINUTES(iml_buffer_validity.GetInt()));
    ...
    if (return_val_sum <= MIN_SUM_FOR_SHORT && return_val_from_rty < 0 && return_val_from_nq < 0 && return_val_from_ym < 0 && return_val_from_es > 0) {
        bool_ready_to_trade_LONG = 1;
        pv_iml_valid_from_dt = sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar];
    }
    // IML context for --------SHORT
    if (return_val_sum >= MIN_SUM_FOR_LONG && return_val_from_rty > 0 && return_val_from_nq > 0 && return_val_from_ym > 0 && return_val_from_es < 0) {
    ...
    if (current_postion.PositionQuantity != 0 && sc.BaseDateTimeIn[sc.Index].GetTime() >= time_to_flat_dt.GetTime()) {
        sc.FlattenAndCancelAllOrders();
    }
    if (sc.BaseDateTimeIn[sc.Index] == 0) {
