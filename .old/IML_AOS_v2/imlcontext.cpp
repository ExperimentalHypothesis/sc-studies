...
SCDLLName("IML AOS")
    ...
    auto prior_bar_dt = static_cast<SCDateTime>(sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] - SCDateTime::MINUTES(nr_of_mins_back.GetInt()));
    ...
    if (sc.Close[sc.IndexOfLastVisibleBar] >= daily_low[sc.IndexOfLastVisibleBar] && sc.Close[sc.IndexOfLastVisibleBar] <= daily_low_mid[sc.IndexOfLastVisibleBar])
        pv_quadrant_nr = LOWEST;
    else if (sc.Close[sc.IndexOfLastVisibleBar] >= daily_low_mid[sc.IndexOfLastVisibleBar] && sc.Close[sc.IndexOfLastVisibleBar] <= mid[sc.IndexOfLastVisibleBar])
        pv_quadrant_nr = SECOND_LOWEST;
    else if (sc.Close[sc.IndexOfLastVisibleBar] >= mid[sc.IndexOfLastVisibleBar] && sc.Close[sc.IndexOfLastVisibleBar] <= daily_high_mid[sc.IndexOfLastVisibleBar])
        pv_quadrant_nr = SECOND_HIGHEST;
    else if (sc.Close[sc.IndexOfLastVisibleBar] >= daily_high_mid[sc.IndexOfLastVisibleBar] && sc.Close[sc.IndexOfLastVisibleBar] <= daily_high[sc.IndexOfLastVisibleBar])
        pv_quadrant_nr = HIGHEST;
    ...
    // display values
    {
        ...
        if (daily_high[sc.IndexOfLastVisibleBar] > daily_high[prior_bar_index] && sc.Close[sc.IndexOfLastVisibleBar] > daily_high_mid[sc.IndexOfLastVisibleBar]) {
            if (display_ut.GetYesNo())
                sc.UseTool(breaking_dh);
            pv_quadrant_nr = BREAK_HIGHEST;
        } else
            sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, 10);
        if (sc.Close[sc.IndexOfLastVisibleBar] < mid[sc.IndexOfLastVisibleBar])
            sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, 10);
        ...
        if (daily_low[sc.IndexOfLastVisibleBar] < daily_low[prior_bar_index] && sc.Close[sc.IndexOfLastVisibleBar] < daily_low_mid[sc.IndexOfLastVisibleBar]) {
            if (display_ut.GetYesNo())
                sc.UseTool(breaking_dl);
            pv_quadrant_nr = BREAK_LOWEST;
        } else
            sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, 11);
        if (sc.Close[sc.IndexOfLastVisibleBar] > mid[sc.IndexOfLastVisibleBar])
            sc.DeleteACSChartDrawing(sc.ChartNumber, TOOL_DELETE_CHARTDRAWING, 11);
