...
SCString log_sc_nq(SCStudyInterfaceRef sc)
{
    ...
    SCString log;
    log.Format("Cena: %f, Date and Time: %i-%i-%i  %0.2i:%0.2i:%0.2i | Daily "
               "range (in ticks): %f | ATR: %0.04f",
...
SCSFExport scsf_MAIN_NQ_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc)
{
    ...
    if (return_val_sum <= min_sum_short && return_val_from_es < 0 && return_val_from_ym < 0 && return_val_from_rty < 0 && return_val_from_nq > 0)
    {
        bool_ready_to_trade_LONG = 1;
        pv_iml_valid_from_dt = sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar];
    }
    // IML context for --------SHORT
    if (return_val_sum >= min_sum_long && return_val_from_es > 0 && return_val_from_ym > 0 && return_val_from_rty > 0 && return_val_from_nq < 0)
    {
